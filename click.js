...
window.addEventListener("DOMContentLoaded", () => {
    ...
    onAuthStateChanged(auth, async (user) => {
        if (user) {
            ...
            if (playerIdDisplay) playerIdDisplay.textContent = userId;
            ...
            if (gameStatus) gameStatus.textContent = "Game is live!";
        } else {
    ...
    function listenToGlobalClicks() {
        onSnapshot(getGlobalDocRef(), (docSnap) => {
            if (docSnap.exists() && totalClicksEl) {
    ...
    function listenToPersonalScore() {
        onSnapshot(getPlayerDocRef(userId), (docSnap) => {
            if (docSnap.exists() && personalScoreEl) {
                const data = docSnap.data();
    ...
    if (clickBtn) {
        clickBtn.addEventListener("click", async () => {
            ...
            try {
                ...
                if (gameStatus) gameStatus.textContent = "✨ +1 Click!";
            } catch (e) {
    ...
    async function handleUserAttack(targetId = null) {
        if (!userId || !db) {
            if (gameStatus) gameStatus.textContent = 'Initialization in progress or user not logged in.';
            return;
        }

        targetId = targetId || targetUserIdInput.value.trim();
        const attackPower = 5;

        if (!targetId || targetId === userId) {
            if (gameStatus) gameStatus.textContent = "Please enter a valid Target Player ID (and not your own!).";
            return;
        }

        if (gameStatus) gameStatus.textContent = `Attacking ${targetId}...`;
        ...
        try {
            await runTransaction(db, async (transaction) => {
                ...
                if (clicksReduced === 0) {
                    if (gameStatus) gameStatus.textContent = `${targetId} already has 0 clicks. Attack failed.`;
                    return;
                }
            ...
            if (gameStatus) gameStatus.textContent = `Attack failed: ${e.message}`;
        }
    }
    ...
    function updateLeaderboardDisplay(data) {
        ...
        document.querySelectorAll('.attack-btn').forEach((btn) => {
            btn.addEventListener('click', async (e) => {
                ...
                if (!targetId) return;
                targetUserIdInput.value = targetId; // Autofill
